...
class GateIoBaseClient(AsyncClient):
        ...
        try:
            ...
            if method == "GET":
                ...
                response = await self.request(
                    "PATCH", url, params=params, data=data, headers=headers
                )
            else:
    ...
    def _get_auth_headers(
        self, method: str, url: str, params: Dict[str, Any], data: Any
    ) -> Dict[str, Any]:
    ...
    def _sign(self, payload) -> str:
        return hmac.new(
            self.API_SECRET_KEY.encode("utf-8"), payload.encode("utf-8"), hashlib.sha512
        ).hexdigest()
