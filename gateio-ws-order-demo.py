...
https://github.com/gateio/gatews/blob/master/python/examples/local_order_book.py
"""
import asyncio
...
from gateio_ws import Configuration, Connection, WebSocketResponse
...
class LocalOrderBook(object):
    ...
    async def ws_callback(self, conn: Connection, response: WebSocketResponse):
        ...
        logger.info("received update: %s", result)
        if response.error:
...
def do_ws_orderbook(loop: AbstractEventLoop):
...
def do_ws_order(loop: AbstractEventLoop):
    app = "futures"
    api_key = "10d23703c09150b1bf4c5bb7f0f1dd2e"
    api_secret = "996c14c32700f3ce63d0f5530793766a7b8bda8cdfaf5d71071915235f3d3f50"
    conn = Connection(Configuration(app=app, api_key=api_key, api_secret=api_secret, test_net=True))
    demo_cp = 'BTC_USDT'
    order_book = LocalOrderBook(demo_cp)
    channel = SpotOrderBookUpdateChannel(conn, order_book.ws_callback)
    # channel.subscribe([demo_cp, "100ms"])
...
def test_login():
    ...
    api_secret = "996c14c32700f3ce63d0f5530793766a7b8bda8cdfaf5d71071915235f3d3f50"
    payload = {}
    param_json = json.dumps(payload)
    message = "%s\n%s\n%s\n%d" % ("api", channel, param_json, data_time)
    ...
    data_param = {
        ...
        "event": "api",
        "payload": {
            "req_header": {"X-Gate-Channel-Id": header},
            "api_key": api_key,
    ...
    print(json.dumps(data_param))
    param = {"time": 1729491063, "channel": "futures.login", "event": "api", "payload": {"req_header": {"X-Gate-Channel-Id": "header"}, "api_key": "10d23703c09150b1bf4c5bb7f0f1dd2e", "timestamp": "1729491063", "signature": "7b1d78f5918c5f18091cd33973ff3e445fac05643c2c8528d8be3e7767577d6bc0c7f8992b31ffc440498c91d13d0a174332c610b8784218b39de507035be1eb", "req_id": "req_id", "req_param": {}}}
